...
TEST(XMLFileHandler, GetSopiuids)
{
	cap::ModelFile xmlFile;
	xmlFile.ReadFile(SAMPLEIMAGES_FILE);
	ASSERT_EQ("SampleImages", xmlFile.name_);

	cap::XMLFileHandler fh(xmlFile);

	std::vector<std::string> sopiuids = fh.GetSopiuids();
	EXPECT_EQ("1.3.12.2.1107.5.2.6.22043.4.0.686845911491386", sopiuids.at(1));
}
