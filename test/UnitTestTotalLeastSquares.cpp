...
#include "../src/CAPTotalLeastSquares.h"
...
//	std::cout << p.normal << ", " << p.position << "\n";
	...
	EXPECT_FLOAT_EQ(p.position.x, 0.0);
	EXPECT_FLOAT_EQ(p.position.y, 0.0);
	EXPECT_FLOAT_EQ(p.position.z, 0.0);
...
//	std::cout << p.normal << ", " << p.position << "\n";
	...
	EXPECT_FLOAT_EQ(p.position.x, -1.0/3.0);
	EXPECT_FLOAT_EQ(p.position.y, 1.0/3.0);
	EXPECT_FLOAT_EQ(p.position.z, 270/6.0);
...
TEST(TotalLeastSquares, ComputeCentroid)
{
	...
	EXPECT_FLOAT_EQ(c.x, 4.0/3.0);
	EXPECT_FLOAT_EQ(c.y, (1+3-2)/3.0);
	EXPECT_FLOAT_EQ(c.z, 1.0/3.0);
}
