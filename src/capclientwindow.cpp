...
void CAPClientWindow::AddImageContours(const std::string& label, const std::vector<ModelFile::Contour>& contours, int frame)
{
	...
	for (; c_it != contours.end(); ++c_it)
	{
		...
		double time = static_cast<double>(frame)/numberOfFrames;
		std::string contour_name = "contour_" + ToString(c.number);
		SetupRegionForContour(cmissContext_, label, contour_name);
		...
		{
			AddContourPoint(cmissContext_, label, time, cps_it->x, cps_it->y);
		}
	}

	int label_index = checkListBox_slice_->FindString(label.c_str());
	bool visibility = checkListBox_slice_->IsChecked(label_index);
	SetVisibilityForGraphicsInRegion(cmissContext_, label, visibility);
}
...
std::vector<Cmiss_field_image_id> CAPClientWindow::CreateFieldImages(const LabelledSlice& labelledSlice)
{
	...
	BOOST_FOREACH(DICOMPtr dicom, labelledSlice.GetDICOMImages())
	{
		Cmiss_field_image_id image_field = CreateImageTexture(field_module, dicom->GetImageSource());
