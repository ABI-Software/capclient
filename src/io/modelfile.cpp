...
std::vector<std::string> ModelFile::GetSopiuids() const
{
	std::vector<std::string> sopiuids;
	std::transform(input_.images.begin(), input_.images.end(),
				   std::back_inserter(sopiuids), boost::bind(&Image::sopiuid, _1));
